...
lm1.Home.score <- lm(Home.Score ~ ., data = train.score[, c(1, 3:20, 40, 43, 44, 47)])
step(lm1.Home.score, trace = 0)
lm2.Home.score <- lm(Home.Score ~ Home_PassAtt + Home_PassTD + Home_RushYds + Home_RushTD + Home_XPA + Home_FGM + Home_FGPercent + Home.O.Rank, data = train.score[, c(1, 3:20, 40, 43, 44, 47)])
...
for(mtry in 1:18) {
  rf=randomForest(Home.Score ~ ., train.score[, c(3:20, 40, 47)], type = "regression", mtry=mtry, ntree=400) 
  oob.err[mtry] = rf$mse[400] #Error of all Trees fitted
  
  pred<-predict(rf, test.score[, c(3:20, 40, 47)]) #Predictions on Test Set for each Tree
  test.err[mtry]= with(test.score[, c(3:20, 40, 47)], mean( (Home.Score - pred)^2)) #Mean Squared Test Error
